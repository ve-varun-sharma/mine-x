...
import { BarChart, File, Mountain, Rss, Shield, Pickaxe, Drill } from 'lucide-react';
import Link from 'next/link';
import { useEffect, useId, useRef, useState } from 'react';
import { EmailButton } from '../ui/Buttons/emailButton';
const tiles = [
    {
        icon: <Pickaxe className="size-full" />,
        bg: (
    ...
    {
        icon: <Drill className="size-full" />,
        bg: (
            <div className="pointer-events-none absolute left-1/2 top-1/2 h-1/2 w-1/2 -translate-x-1/2 -translate-y-1/2 overflow-visible rounded-full bg-gradient-to-r from-orange-600 via-rose-600 to-violet-600 opacity-70 blur-[20px] filter"></div>
        )
    },
                    ...
                    <div className="relative flex w-full flex-col items-center justify-center overflow-hidden">
                        ...
                        <div className="absolute z-10">
                            <div className="mx-auto size-24 rounded-[2rem] border bg-white/10 p-3 shadow-2xl backdrop-blur-md dark:bg-black/10 lg:size-32">
                                <Pickaxe className="mx-auto size-16 text-black dark:text-white lg:size-24" />
                            </div>
                            <div className="z-10 mt-4 flex flex-col items-center text-center text-primary">
                                <h1 className="text-3xl font-bold lg:text-4xl">
                                    {' '}
                                    Empower Your Mining Company with Digitial Transformation & Cutting-Edge Technology.
                                </h1>
                                <p className="mt-2">Contact us to see how we can help.</p>
                                {/* <Link
                  href="#"
                  className={cn(
                    buttonVariants({
